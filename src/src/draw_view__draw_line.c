...
/*   Updated: 2025/02/13 00:58:15 by katakada         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_line_start_to_end(t_vertex_fdf start_raws,
		t_vertex_fdf end_raws, t_view *view, t_image *image)
{
	...
	if (end_fdf == NULL)
		forced_error_exit("fdf model data is invalid", __FILE__, __LINE__);
	start_dot = convert_fdf_to_view_dot(*start_fdf, view);
		...
		t_image *image)
{
	...
	draw_line_start_to_end(start_raws, end_raws, view, image);
}
